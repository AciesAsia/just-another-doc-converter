...
=============================================================
  ...
  Usage:
    python pdf_to_office_gui.py       (standalone; launcher preferred)
...
class PdfToOfficeApp(tk.Frame):
    ...
    def _build_ui(self):
        ...
        if _DND_AVAILABLE:
            tk.Label(
                queue_hdr, text="(drag & drop PDF files here)",
                bg=CLR_BG, fg=CLR_SUBTEXT,
        ...
        # F-10/F-19: register queue listbox as DnD drop target
        if _DND_AVAILABLE:
        ...
        self._btn_open = self._make_btn(
            mid, "\U0001f4c2  Open Output Folder", CLR_SURFACE, CLR_BORDER,
            ...
            if path.is_file() and path.suffix.lower() == ".pdf":
                ...
                for f in sorted(path.iterdir()):
                    if (f.is_file() and f.suffix.lower() == ".pdf"
                        ...
                        added += 1
        if added:
            self._log_write(f"Dropped {added} PDF file(s) into queue.\n", "info")
        self._refresh_queue()
    ...
    def _start_conversion(self):
        ...
        # F-05: status update from main thread — safe
    ...
    def _run_conversions(self, files, mode):
        ...
        # Routes to the GUI via self.after() AND calls the original logger.
        ...
        for i, src in enumerate(files, 1):
            ...
            if mode == "excel":
                dest = conv._safe_dest(output_dir, src.stem, ".xlsx")  # F-07
                converter_fn = conv.convert_pdf_to_xlsx
            else:
                dest = conv._safe_dest(output_dir, src.stem, ".docx")  # F-07
                converter_fn = conv.convert_pdf_to_docx

            # F-20: pass log_fn instead of monkey-patching conv.log
            success = converter_fn(src, dest, log_fn=_gui_log)
        ...
        # F-24: close_office_apps() not called here; registered as atexit handler in converter.py
...
#        plain Tk otherwise; packs PdfToOfficeApp (now a Frame) into it.
